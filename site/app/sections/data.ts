...
export const ROADMAP: Phase[] = [
  ...
  { n: "30", title: "Vault import (CSV + JSON)", subtitle: "The symmetric pair to the export: bulk-add holdings from a CSV or JSON file (one exported above, or a spreadsheet kept elsewhere) into the vault, with honest skip-reporting. Rows are inserted directly as holdings (not via the add path that tops-up and stamps acquired_at=now), so the imported purchase date is preserved and the Row 27 acquisition timeline stays accurate. A row whose card_id isn't in the catalog, or is missing, or has a quantity below 1, is skipped with a reason — never silently dropped or coerced; optional empty fields are null, never a fabricated $0. An over-cap file is rejected before any write. Shipped as POST /collection/import?format=csv|json (literal, registered before the parametric PATCH) + a VaultImport panel in the Vault (file picker + paste, honest skip table) (no data/ writes)", status: "done" },
  { n: "29", title: "Sold-lot ledger (realized gains)", subtitle: "The disposal counterpart to the vault: a permanent, append-only record of cards you've *sold*. Each lot carries its sale price, optional fee, and the cost basis *snapshotted at sale time*, so realized P/L is fixed and never recomputed against a holding you may have since edited or deleted. Honest: proceeds = (sale price − fee) × quantity is always known (a sale has a price); cost basis / realized are null when no purchase price was recorded — never a fabricated $0; the summary's total_realized is computed over the cost-known subset only, lots without one counted but excluded, never $0; winners/losers counted; an empty ledger shows an empty state, not a block of zeros. A holding row's 'Log sale' button pre-fills the form (card id, variant, what you paid). Shipped as SoldLot model + SoldLotService + GET /sold-lots, GET /sold-lots/summary, POST /sold-lots, DELETE /sold-lots/{lot_id} + a SoldLedger panel in the Vault (mounted unconditionally — you can sell your last card; no data/ writes)", status: "done" },
  ...
  { n: "31", title: "Two app modes (Key + Full)", subtitle: "A curated flagship 'Key' mode (the six core collector-loop features — Scan, Vault, Binder, Sets, Sealed, Deals — front and centre, everything else under More, lands on Scan) and an unchanged 'Full' mode with all 15 tabs, toggled from the More tab and persisted to localStorage · config-driven nav so both modes render from one source, every non-key surface still reachable (deep link / command palette) with More highlighted, never hidden · Phase A infrastructure shipped; per-feature polish of the six key surfaces is Phase B", status: "progress" },
  { n: "32", title: "Phase B — key-feature polish", subtitle: "Take each of the six Key-mode surfaces to its best form, one feature per shipped change — all six done. Vault: the eight stacked panels buried the holdings table — reordered so the table is primary content under the KPIs, action tools (export / import / sold-ledger) grouped in an open Tools section, and the five read-only analytics panels grouped in a collapsible Analytics section (default open) · shipped (commit d639cb0). Scan: the confirm-this-is-the-right-card + what-you-paid loop was buried under five grading panels — surfaced it up top and grouped the deep grading/centering/studio/trade-up/authenticity analysis in a collapsible Grading & analysis section (default open) · shipped (commit b9638e0). Binder: a show-off-with-proof surface (not a wall), so the polish is informational — a proven-coverage summary in the toolbar (N cards · M of N proven, honest counts never a summed $) and a first-run 3-step orientation in the empty state · shipped (commit 12ae99d). Sets: the set-completion tool's core loop is 'open a set → see what you're missing → chase it', but the checklist showed every card — added a 'Show only missing' filter (default off, gated off for complete sets, reset on set change) so a collector sees just the gaps immediately · shipped (commit 840a2a3). Sealed: the flip-edge sniper's ranked feed mixed real flips with 'not a deal' context cards — added an at-a-glance 'N listings · M flips' summary (honest counts, never a summed $) and a 'Show only flips' focus filter (default off, hidden when there are no flips, reset on new search) · shipped (commit 90e734d). Deals: the card sniper, structurally parallel to Sealed — same summary + 'Show only deals' filter (a deal = rip OR flip, honest counts never a summed $, default off, hidden when nothing is a deal, reset on new results) · shipped", status: "done" },
];
