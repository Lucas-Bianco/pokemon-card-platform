...
export const ROADMAP: Phase[] = [
  ...
  { n: "18", title: "Collection insurance value", subtitle: "Sum latest_price across the vault in a conservative/median/aggressive band + a printable schedule — the thing a serious collector actually needs; honest 'no price → excluded' never $0; reuses the price layer", status: "planned" },
  { n: "19", title: "Trade-up / sell-now simulator", subtitle: "Pick cards you own → sell-raw-now (eBay sold-median − fees) vs grade-then-sell (graded-price − grading fee − time risk), using the graded-price provider + Grading Studio band; surfaces the honest 'grading only pays if centering ≥ 8' cutoff", status: "planned" },
  { n: "20", title: "Price-alert thresholds (pull, not push)", subtitle: "Set a target price per card; the existing snapshot pipeline flips an alert when a new sold-comp crosses it — reuses the alerts table + sold-comps provider, no SMS infra, no false 'we'll notify you' promise", status: "planned" },
  { n: "21", title: "Shareable binder page", subtitle: "A public-ish read-only view of a curated subset of your vault, each card backed by its proven sold-comp — honest marketing for the platform + a way to show off a PC", status: "planned" },
  { n: "22", title: "Market trend charts", subtitle: "Per-card price history charts from the append-only snapshot log — blocked on a snapshot-retention decision (today snapshots are trimmed); honest about the gap, not a magic trend line", status: "planned" },
