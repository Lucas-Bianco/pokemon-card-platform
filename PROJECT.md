...
| 9 | Sealed-product catalog + MSRP — curated master catalog (ETB / booster box / pack / premium) with product_type, MSRP, normalized name, set link | **Complete (backend + component)** — `SealedProduct` model + `SealedCatalogService` + 24-row curated in-repo seed, MSRP nullable/"no MSRP" never $0, in_print/out_of_print tags, `GET /sealed/products` routes + `SealedCatalog.tsx`; curated seed NOT magic auto-update, semi-automated community sync is a documented follow-up. Shipped PARTIAL 2026-08-22 (commit 34cf206, backend+component only); Catalog tab mount + B/C/D shipped 2026-08-22 |
| 10 | Scan-to-log sealed products — camera OCR of box/pack text → match catalog → pre-fill the purchase ledger | **Complete (catalog-driven form)** — `POST /sealed/ledger/from-catalog` logs a buy by catalog slug (name + product_type resolved server-side); inline "Log to ledger" mini-form on each Catalog card (quantity + cost + optional source) → toast. The camera-OCR scan-to-box-text match is a future follow-up; the honest catalog-driven log ships now. Shipped 2026-08-22 |
| 11 | MSRP vs market view — MSRP (from 9) vs live market (existing eBay sold-median), searchable + scannable | **Complete** — `GET /sealed/products/{slug}/market` returns curated MSRP vs live sold-comps median + delta + the SAME honest unavailable/empty flags as /sealed/sold-comps; inline "vs market" panel on each Catalog card. `delta` null unless BOTH msrp + median real; provider failure degrades to [] (never 5xx). Shipped 2026-08-22 |
| 12 | Price lookup by name — type a card name → prices across variants/sets, no scan needed | **Complete** — `GET /cards/lookup?q=&limit=` (func.lower().like(), NOT ilike) + `CardLookupService` reusing `PriceService.latest_price`; new Prices tab (`PriceLookup.tsx`, debounced 300ms) with honest "no market price" (em dash, never $0) + source/staleness per row. Shipped 2026-08-22 |
...
| 16 | Proof of sales — every market price backed by viewable sold-comps (date, price, source link) | Done 2026-08-21 — new `GET /sealed/sold-comps` (query-keyed, on-demand, EndedWithSales gate) + reusable `ProofOfSales.tsx` under ScanResult's price (card) + per-row "Show proven sales" toggle on SealedDeals/SealedLedger (sealed); listed-vs-proven caveat always shown; honest `sold_comps_unavailable`/`sold_comps_empty`, never $0; 615 backend + 195 frontend tests green, 105-scan baseline untouched |
...
| 18 | Vending-machine restock tracker — work out when a Pokémon vending machine actually restocks | Planned — no API exists for this, so the honest version is a log of the owner's own sightings (`restocked` / `still empty`) plus a described pattern, never a predicted time. Reports a WINDOW with `n` and an `insufficient_data` floor at 3 restocks; `empty` sightings bound each restock by interval censoring so a weekly shopper does not "learn" their own shopping day. Reuses the existing `drop_time` watch + `subject_label` (no new alert type); arming stays manual. [spec](docs/superpowers/specs/2026-08-22-vending-restock-design.md) |
