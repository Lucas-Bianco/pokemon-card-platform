...
## Phase B — Vault polish: reorganize the panel stack (shipped 2026-08-28)

First Phase B feature polish (per the Row 31 plan). The key-mode Vault had grown to **eight panels stacked between the headline KPIs and the holdings table** — InsuranceValue, Diversification, PortfolioHistoryChart, PriceFreshness, AcquisitionTimelineChart, VaultExport, VaultImport, SoldLedger — which buried the thing a collector actually acts on (edit a price, open a history chart, remove a row, log a sale) under a wall of read-only analytics. Reordered for information hierarchy — **primary content leads**:

  KPIs + caveats → at-a-glance (allocation / top movers) → **holdings table** → **Tools** section → **Analytics** (collapsible)

  - **Holdings table** now sits right under the KPIs and at-a-glance summaries (was at the very bottom).
  - **Tools** `<section className="vault-section vault-tools" aria-label="Vault tools">` with `<h2>Tools</h2>` groups the three **action** surfaces — `VaultExport` (still `hasHoldings`-gated), `VaultImport` (still unconditional — you import to populate an empty vault), `SoldLedger` (still unconditional — you can sell your last card). Kept **open, not collapsible**: a holding row's "Log sale" button lifts `salePrefill` and `SoldLedger`'s prefill `useEffect` opens the form + `scrollIntoView`s to it; a closed container would hide that.
  - **Analytics** `<details className="vault-section vault-analytics" open>` with a styled `<summary>Analytics</summary>` groups the five **read-only** panels — InsuranceValue, Diversification, PortfolioHistoryChart, PriceFreshness, AcquisitionTimelineChart. Collapsible so a collector who just wants their holdings can fold the wall away; **default `open`** keeps the data visible without an extra tap and keeps every panel mounted so the honest-price invariants and existing assertions are unchanged. All five stay `hasHoldings`-gated.

All eight panels still mount, the `salePrefill` flow is unchanged, and the existing `PortfolioView.test.tsx` assertions (query by class/text, never by order) all still pass. New CSS `.vault-section` (top margin) / `.vault-section > h2` (uppercase dim label) / `.vault-analytics > summary` (cursor pointer, uppercase dim, `list-style:none`, hides the default `::-webkit-details-marker`, renders a `▾`/`▸` glyph via `::before` that swaps on `:not([open])`) — appended after `.import-skip-table`, before `.freshness-bands`.
